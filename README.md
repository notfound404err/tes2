...
## Live chat (Telegram relay)

This project includes a simple live-chat popup in the website that forwards messages to your Telegram account via a small Express relay server. The server records the incoming request IP address and attaches a timestamp before sending each message to Telegram.
...
- Install dependencies and start the relay server (PowerShell):
...
- Open the site (e.g. with `npm run dev` or open `index.html`) and use the chat button at bottom-right to send messages.

### Notes & security

- The relay server must run so the frontend can POST messages to `http://localhost:3000/send-message` by default. If you host the relay elsewhere, update `script/chat-popup.js` `SERVER_URL` constant.
- Keep `BOT_TOKEN` private. Do not commit `.env` to public repositories.
- The relay attaches the request IP (from `x-forwarded-for` or socket) and a local timestamp when forwarding to Telegram.
